...
test("TC02 - Declined payment does not confirm booking", async ({ page }) => {
  /**
   * Step 0: Test setup
   *
   ...
   * Step 6: Payment
   *
   * - Fill Declined test card data to verify appointment is not successful
   */
  const paymentPage = new PaymentPage(page);
  await paymentPage.pay(testData.declinedPayment);
  await paymentPage.continue();

  await paymentPage.expectCardDeclinedError();
